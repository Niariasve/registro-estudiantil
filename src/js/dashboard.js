const DashboardModule = (() => {
  ...
  function renderStudentTable(items) {
    const searchInput = byId('searchDashboardStudent');
    const filterSelect = byId('filterDashboardStatus');
    ...
    if (statusFilter === 'evaluated') {
      filteredItems = filteredItems.filter((item) => item.average !== null);
    } else if (statusFilter === 'pending') {
      filteredItems = filteredItems.filter((item) => item.pendingGrades > 0);
    }
    ...
    const emptyText = query || statusFilter !== 'all'
  ...
  function renderCharts(metrics) {
    if (typeof Chart === 'undefined') {
      const msg = byId('chartsMessage');
      if (msg) msg.textContent = 'No se pudo cargar Chart.js. Las tablas y exportaciones CSV siguen disponibles.';
      return;
    }
    ...
    if (!classCanvas || !distributionCanvas) return;
  ...
  function bindEvents() {
    const exportClassBtn = byId('exportClassChart');
    if (exportClassBtn) {
    ...
    const exportDistBtn = byId('exportDistributionChart');
    if (exportDistBtn) {
      exportDistBtn.addEventListener('click', () => {
        exportChart(distributionChart, 'distribucion-estudiantes.png');
      });
    }

    const exportCsvBtn = byId('exportDashboardCsv');
    if (exportCsvBtn) {
